        ...
        collector.on("collect", async (message) => {
            if (message.customId === "loop") {
                if (!player) {
                    ...
                    const embed = new EmbedBuilder().setDescription(`\`✔️\` | Loop mode set to : \`${player.loop}\``).setColor(clientOptions.embedColor);
                    message.reply({ embeds: [embed], ephemeral: true });
                    const loopEmbed = new EmbedBuilder()
                    .setAuthor({
                        name: `Now Playing`,
                ...
                if (!player) {
                    ...
                    await player.seek(0);
                    const embed = new EmbedBuilder().setDescription('\`✔️\` | The song has been replayed').setColor(clientOptions.embedColor);
                ...
                if (!player) {
                    ...
                    const embed = new EmbedBuilder().setDescription(`\`❌\` | Queue is: \`Empty\``).setColor(clientOptions.embedColor);
                ...
                await message.reply({ 
                    embeds: [new EmbedBuilder().setDescription(`\`✔️\` | Volume decreased to : \`${player.volume}%\``).setColor(clientOptions.embedColor)], 
                    ephemeral: true 
                });
                ...
                await message.reply({ 
                    embeds: [new EmbedBuilder().setDescription(`\`✔️\` | Volume increased to : \`${player.volume}%\``).setColor(clientOptions.embedColor)], 
                    ephemeral: true 
                });
